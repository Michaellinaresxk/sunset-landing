...
import { EXPERIENCE_STEPS, EXPERIENCE_BG } from '@/constants';
...
export default function ExperienceSection() {
  return (
    <SectionReveal className='py-20 md:py-32 relative overflow-hidden'>
      {(isVisible) => (
        <>
          <div className='absolute inset-0 bg-cover bg-center' style={{ backgroundImage: `url(${EXPERIENCE_BG})` }} />
          ...
          <div className='max-w-7xl mx-auto px-4 md:px-8 relative z-10'>
            <RevealItem isVisible={isVisible} className='text-center mb-12 md:mb-20'>
              <h2 className='text-4xl md:text-6xl lg:text-7xl font-extralight text-white mb-4 tracking-tighter'>
                The Journey
              </h2>
              <p className='text-base md:text-lg text-white/70 font-light'>Every moment thoughtfully orchestrated</p>
            </RevealItem>
              ...
              {EXPERIENCE_STEPS.map((exp, i) => (
                <RevealItem key={i} isVisible={isVisible} delay={i * 100}>
                  <div className='bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20 shadow-xl'>
                    ...
                    <h3 className='text-base font-light text-white mb-1 leading-tight'>{exp.title}</h3>
                    <p className='text-xs text-white/70 font-light leading-relaxed'>{exp.desc}</p>
                  </div>
                </RevealItem>
              ))}
            </div>
              ...
              {EXPERIENCE_STEPS.map((exp, i) => (
                <RevealItem key={i} isVisible={isVisible} delay={i * 150}>
                  <div className='bg-white/10 backdrop-blur-md rounded-3xl p-6 border border-white/20 shadow-xl transition-all duration-1000 hover:scale-105 hover:bg-white/15 hover:shadow-2xl hover:shadow-amber-500/30'>
                    ...
                    <h3 className='text-xl font-light text-white mb-3 tracking-tight leading-tight'>{exp.title}</h3>
                    <p className='text-sm text-white/70 font-light leading-relaxed'>{exp.desc}</p>
                  </div>
                </RevealItem>
              ))}
            </div>
