...
export default function HeroSection({ onBookClick }: SectionProps) {
  ...
  return (
    <div className='relative h-[75vh] md:h-screen min-h-[500px] md:min-h-[700px] overflow-hidden'>
      {/* Parallax Background */}
      <div
        className='absolute inset-0'
        style={{ transform: `translateY(${parallax}px)` }}
      ...
      {/* Content */}
      <div className='relative z-10 h-full flex items-end pb-14 md:pb-32'>
        <div className='max-w-7xl mx-auto px-4 md:px-8 w-full'>
          <div className='max-w-4xl space-y-4 md:space-y-8'>
            {/* Badge */}
            <div className='inline-flex items-center gap-2 px-3.5 py-1.5 md:px-5 md:py-2.5 rounded-full bg-white/5 backdrop-blur-2xl border border-white/10'>
              <div className='w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-amber-300 animate-pulse' />
              <span className='text-white/90 text-[10px] md:text-sm tracking-widest uppercase font-light'>
                Exclusive Sunset Experience
              </span>
            </div>

            {/* Title */}
            <h1 className='text-5xl sm:text-7xl md:text-8xl lg:text-9xl font-extralight text-white tracking-tighter leading-none'>
              Golden
              <span className='block text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-orange-300 to-amber-200 mt-1 md:mt-4'>
                Hour
              </span>
            </h1>

            <p className='text-sm md:text-2xl lg:text-3xl text-white/60 font-light max-w-2xl leading-relaxed'>
              Where paradise meets perfection, captured in nature&apos;s most
              spectacular light
            </p>
      ...
      <div className='hidden md:block absolute bottom-12 left-1/2 -translate-x-1/2 animate-bounce'>
        <ChevronDown className='w-8 h-8 text-white/30' />
      </div>
    </div>
  );
