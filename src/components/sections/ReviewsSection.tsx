...
const REVIEWS = [
  ...
  },
  {
    ...
    initials: 'PO',
    country: '🇬🇧',
    text: 'Had a terrific visit for our annual family reunion. The staff was very friendly and accommodating. Food was excellent — the octopus soup was a big hit. Punta Cana has pretty much anything you could want whether you are being active or just laying around. All in all we will come again.',
  },
  {
    id: 3,
    name: 'Jennifer Knopf',
    ...
    initials: 'JK',
    country: '🇫🇷',
    text: 'We had a wonderful experience on a private tour of Monkey Land! The place is beautiful, the service warm and enthusiastic. The monkeys are clearly well cared for and happy. Highly recommend!',
  },
  {
    id: 4,
    name: 'Austin Gould',
    ...
    initials: 'AG',
    country: '🇨🇦',
    text: 'Chef Jose truly went above and beyond for us. He quickly learned our desired portion sizes, was always flexible with meal times, and made some incredible food. Each dish was unique and full of flavor. I cannot recommend Jose enough!',
  },
  {
    id: 5,
  ...
  },
  {
    ...
    initials: 'CV',
    country: '🇺🇸',
    text: 'The staff was great, we all really enjoyed our visit. The chef was amazing, food was delightful. The maids were super hands on with cleaning and washing for us. I would definitely recommend staying with them and will definitely be coming back to visit.',
  },
];
...
function ReviewCard({ review }: { review: (typeof REVIEWS)[0] }) {
  return (
    ...
    >
      <div className='flex flex-col gap-4 p-5 md:p-6 rounded-2xl bg-white/[0.03] border border-white/[0.08] group-hover:border-amber-500/30 group-hover:bg-white/[0.06] transition-all duration-300 h-full cursor-pointer'>
...
export default function ReviewsSection() {
  ...
  const updateState = useCallback(() => {
    ...
    setActiveIndex(Math.round(el.scrollLeft / (itemWidth + 12)));
  ...
  const scrollTo = useCallback((direction: 'prev' | 'next') => {
    ...
    el.scrollBy({
      left: direction === 'next' ? itemWidth + 12 : -(itemWidth + 12),
      behavior: 'smooth',
    });
  }, []);
            ...
            <RevealItem isVisible={isVisible} delay={300}>
              <div className='max-w-6xl mx-auto px-4 md:px-8 mt-6 flex flex-col sm:flex-row items-center justify-between gap-4'>
                ...
                <a
                  ...
                  target='_blank'
                  rel='noopener noreferrer'
                  className='inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full border border-white/10 bg-white/[0.03] hover:bg-white/[0.07] hover:border-white/20 transition-all duration-300 group'
                >
                  <span className='flex items-center gap-0.5'>
